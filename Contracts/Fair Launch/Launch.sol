...
contract Launch {
    ...
    function createNewICO(DataParam calldata newICOData) public virtual{
        ...
        // Transfer Token
        token.transferFrom(msg.sender, address(icoContract), _totalFees(newICOData.liquidityPercent, newICOData.softCap, newICOData.presaleSupply));
    ...
    function _createAndAssign(ICOparam memory icoParam, ICO icoContract) internal {
     ...
     function _totalFees(uint256 _liquidityPercent, uint256 _softCap, uint256 _presaleSupply) internal virtual returns(uint256){
        // uint256 extraTokenForLiquidity =  _liquiditySupply * _hardCap * _exchangeListingRateBNB / 10000;
        ...
        uint256 listingRate = _presaleSupply/_softCap;
        uint FEE = ICOAdminWalletFees * _softCap/10000;
        _softCap -= FEE;
        uint liquiditySupply = _liquidityPercent * _softCap * listingRate;
        uint256 extraTokenForAdminFees = _presaleSupply.mul(ICOStakingPoolFees).div(10000);
        uint256 totalFees = _presaleSupply.mul(ICOAdminWalletFees).div(10000);

        return (liquiditySupply + _presaleSupply + extraTokenForAdminFees + totalFees);
    }
